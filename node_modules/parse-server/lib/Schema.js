...
var operationForbiddenError = function operationForbiddenError(className, operation) {
  return new Parse.Error(Parse.Error.OPERATION_FORBIDDEN, 'This user is not allowed to perform the ' + operation + ' operation on ' + className + '.');
};

var incorrectPrivateFieldTypeError = { code: Parse.Error.INVALID_JSON, error: 'Private must be a boolean.' };
...
// '_metadata' is CLPs, private fields, and other stuff.
...
var Schema = function () {
  function Schema(collection) {
    ...
    this.perms = {};
    // this.privateFields[className] gives you a list of the private fields
    this.privateFields = {};
  }
  ...
  _createClass(Schema, [{
        ...
        try {
          var _loop = function _loop() {
            ...
            Object.keys(obj).forEach(function (key) {
              ...
              switch (key) {
                ...
                case '_metadata':
                  ...
                  if (value && value.private_fields) {
                    privateFields = value.private_fields;
                  }
                  break;
                default:
                  classData[key] = value;
            ...
            if (className) {
              ...
              _this.privateFields[className] = privateFields;
              if (permsData) {
    ...
    value: function updateClass(className, submittedFields, classLevelPermissions, database) {
      ...
      var newlyPrivateFields = [];
      var newlyNonPrivateFields = [];
      Object.keys(submittedFields).forEach(function (fieldName) {
        var field = submittedFields[fieldName];
        if (submittedFields[fieldName].private) {
          if (typeof submittedFields[fieldName].private !== 'boolean') {
            throw new Parse.Error(Parse.Error.INVALID_JSON, 'Private must be a boolean.');
          }
          if (submittedFields[fieldName].private) {
            newlyPrivateFields.push(fieldName);
          } else {
            newlyNonPrivateFields.push(fieldName);
          }
        }
        if (existingFields[fieldName] && 'type' in field) {
          throw new Parse.Error(255, 'Field ' + fieldName + ' exists, cannot update.');
        }
        if (!existingFields[fieldName] && field.__op === 'Delete') {
      ...
      mongoObject = mongoObject.result;
      ...
      .then(function () {
        return Promise.all(insertedFields.filter(function (fieldName) {
          return !!mongoObject[fieldName];
        }).map(
        // Insert the fields into the schema using the horribly named "validateField"
        function (fieldName) {
          return _this2.validateField(className, fieldName, mongoObject[fieldName]);
        }));
      }).then(function () {
        return _this2.setPermissions(className, classLevelPermissions);
      }).then(function () {
        return _this2.setFieldPrivacy(newlyPrivateFields, newlyNonPrivateFields);
      }).then(function () {
        return mongoSchemaToSchemaAPIResponse(mongoObject);
      });
    }
    ...
    // Sets the field privacy in the database

  }, {
    key: 'setFieldPrivacy',
    value: function setFieldPrivacy(newlyPrivateFields, newlyNonPrivateFields) {
    ...
    value: function validatePermission(className, aclGroup, operation) {
      ...
      var operationPermitted = false;
      for (var i = 0; i < aclGroup.length && !operationPermitted; i++) {
        if (perms[aclGroup[i]]) {
          operationPermitted = true;
        }
      }
      if (!operationPermitted) {
        throw operationForbiddenError(className, operation);
      }
    }
  }, {
    key: 'getExpectedType',
...
function mongoSchemaFromFieldsAndClassNameAndCLP(fields, className, classLevelPermissions) {
  ...
  for (var _fieldName in defaultColumns[className]) {
    var schemaAPIType = defaultColumns[className][_fieldName];
    if (schemaAPIType.private) {
      if (typeof schemaAPIType.private !== 'boolean') {
        return incorrectPrivateFieldTypeError;
      }
      if (schemaAPIType.private) {
        privateFields.push(_fieldName);
      }
    }
    var validatedField = schemaAPITypeToMongoFieldType(schemaAPIType);
    if (!validatedField.result) {
      return validatedField;
    }
    mongoObject[_fieldName] = validatedField.result;
  }

  for (var _fieldName2 in fields) {
    var _schemaAPIType = fields[_fieldName2];
    if (_schemaAPIType.private) {
      if (typeof _schemaAPIType.private !== 'boolean') {
        return incorrectPrivateFieldTypeError;
      }
      if (_schemaAPIType.private) {
        privateFields.push(_fieldName2);
      }
    }
    if ('type' in _schemaAPIType) {
      var _validatedField = schemaAPITypeToMongoFieldType(_schemaAPIType);
      if (!_validatedField.result) {
        return _validatedField;
      }
      mongoObject[_fieldName2] = _validatedField.result;
    }
  }
  ...
  if (privateFields.length > 0) {
    ...
    mongoObject._metadata.private_fields = privateFields;
  }
...
function mongoSchemaAPIResponseFields(schema) {
  ...
  fieldNames.forEach(function (fieldName) {
    ...
    if (schema._metadata && schema._metadata.privateFields) {
      if (schema._metadata.privateFields.indexOf(fieldName) !== -1) {
        response[fieldName].private = true;
      }
    }
  });
...
function mongoSchemaToSchemaAPIResponse(schema) {
  ...
  if (schema._metadata && schema._metadata.private_fields) {
    schema._metadata.private_fields.forEach(function (fieldName) {
      result.fields[fieldName].private = true;
    });
  }
